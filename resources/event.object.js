...
(function () {
  ...
  /* Список объектов, требующих отслеживания */
  let object_list = [];
  ...
  const editAppView = new MutationObserver(() => {
    ...
    /* Добавим кнопку добавления объекта в любимые */
    if (settingMaster["favorite-objects"]) {
      ...
      if (headerObject.length) {
        ...
        if (!headerObject.find(".nk-button-favorite-object").length) {
          ...
          popupShow(button, !activeStatus ? chrome.i18n.getMessage("addFavoriteObject") : chrome.i18n.getMessage("removeFavoriteObject"));
          button.on("click", () => {
            const activeStatus = settingFavoriteObjects["user-objects"][nameObject] !== undefined;
    ...
    /* Ищим объект в списке отслеживаемых, если такого вида объекта нет в списке, то ставим его в стоп-лист */
    let ID_setting = -1;
    check = true;
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (!settingMaster["object:" + setting.category]) {
          stopList.push(ID);
          return;
        }

        if (setting.category === "bld" && !settingMaster.fr_mos_ru) {
          stopList.push(ID);
          return;
        }
  ...
  const initEventObject = (setting) => {
    ...
    window.appChrome.settingMaster = settingMaster;
  };
